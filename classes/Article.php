...
class Article
{
     ...
     * 
     * @return array An associative array of the page of articles
     */
    ...
    /**
     * Get the article record based on the ID along with associated categories, if any exist.
     * 
     * @param object $conn Connection to the database
     * @param integer $id the article ID
     * 
    ...
    /**
     * Validate the properties, putting any validation error messages in the $errors property article properties    *      
     * 
    ...
    protected function validate() 
    {
        ...
        if ($this->published_at != '') {                                 
            ...
            if ($date_time === false) {
                ...
                // Check to ensure date itself is valid (eg. 22-13-32 would be an invalid date)
    ...
    public function create($conn)
    {
        if ($this->validate()) {
            ...
            if ($this->published_at == '') {
                // If no date is provided, set date to null
                $stmt->bindValue(':published_at', null, PDO::PARAM_NULL);
            } else {
                $stmt->bindValue(':published_at', $this->published_at, PDO::PARAM_STR);
            }
            ...
            if ($stmt->execute()) {
                // Get the id of the article that has just been created from the database
